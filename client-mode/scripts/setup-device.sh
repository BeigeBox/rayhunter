...
adb push "$SCRIPT_DIR/run-wifi.sh" /data/rayhunter/scripts/run-wifi.sh
...
echo ""
echo "Files pushed. Now create credentials on the device:"
echo "  adb shell"
echo "  cat > /data/rayhunter/wifi-creds.conf <<EOF"
echo "  ssid=YourNetworkName"
echo "  password=YourPassword"
echo "  EOF"
echo ""
echo "Then reboot and trigger with:"
echo "  adb shell \"rootshell -c 'printf \\\"\\\\r\\\\nAT+SYSCMD=sh /data/rayhunter/scripts/run-wifi.sh\\\\r\\\\n\\\" > /dev/smd8'\""
